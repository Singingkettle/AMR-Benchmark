...
def calculate_acc_cm_each_snr(Y, Y_hat, snrs, cm, classes=None, save_figure=True, min_snr=0):
    ...
    for snr in snrs:
        ...
        print(min_snr)
        if snr >= min_snr:
            plot_confusion_matrix(cm, title='Confusion matrix at {}db'.format(snr), cmap=plt.get_cmap("Blues"),
                                  labels=classes, save_filename='figure/cm_snr{}.png'.format(snr))
